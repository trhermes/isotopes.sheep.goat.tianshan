...
FD1=function(x,y){A*cos(2*pi*((x-x_0)/z))+M}
...
p1 <- ggplot(data = Estims_plot, aes(x = x, y = median_est, ymin = ci_lower_est, ymax = ci_upper_est)) + 
  ...
  scale_y_continuous(expand=c(0,0),limits=c(-14, 2),
                     ...
                     minor_breaks = NULL) +
  scale_x_continuous(expand=c(0,0),limits=c(-40,0),labels = c("40", "30", "20", "10", "0"),
...
}else{curvemin=optimize(FD1, interval=c(min(xdata),max(xdata)))}
...
}else{curvemax=optimize(FD1, interval=c(min(xdata),max(xdata)), maximum=TRUE)}
...
corr_tests <- all_data_comp %>% 
  select(d13C, d18O, specimen, site) %>% group_nest(specimen,site) %>% 
...
ggsave("../Figures/Fig S1 - scatter plots of d13C and d18O values by tooth.pdf",corr_tests_plot,dpi = 300,
       width = 20, height = 30, units = c("cm"), scale = .85)

# Birth seasonality chart for all sites -- all_data_comp
birth <- all_data_comp %>% filter(element == "M/2", increment == 1) %>% distinct(increment, specimen, .keep_all = T)
...
ggsave("../Figures/birth_seasonality_plot_caprines.png", birth_plot, dpi = 300,
